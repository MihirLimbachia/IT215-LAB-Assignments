...
{    
    ...
    if(*argv!=NULL && builtin_cmd(argv)){
               ...
               Sigaddset(&mask, SIGSTOP);               // adding SIGSTOP signal to the set mask
               Sigprocmask(SIG_BLOCK, &mask, NULL);     // blocking the set so that the parent does not recieve SIGCHLD SIGINT or SIGSTOP signal before creating this job process is initiated in child process
              if((cpid=fork())==0){                     // creating child process to execute non-builtin command
             ...
             if(execve(argv[0],argv,environ)<0){        // executing the non-builltin command
   ...
   if(!bg){
               ...
               Sigprocmask(SIG_UNBLOCK, &mask,NULL);      // unblocking for parent process after adding the job as now the child is added in jobs table and now the parent is ready to recieve signals
            ...
            Sigprocmask(SIG_UNBLOCK, &mask, NULL);       // unblocking for parent process after adding the job as now the child is added in jobs table and the parent is ready to recieve signals                   
...
void do_bgfg(char **argv) 
{
        ...
        struct job_t *jb=NULL;
         if(strcmp("fg",argv[0])==0 || strcmp("bg",argv[0])==0){                             // checking if it is fg or bg 
             if(argv[1]==NULL){                                                              // checking if we have anthing as an input after fg or bg command
     ...
     {
       jb->state=FG;                                                                           //set the state of foreground jobs
       waitfg(jb->pid);                                                                        //waiting for foreground jobs to terminate
     }
     else
     {
      jb->state =BG;                                                                           //set the state of background jobs 
